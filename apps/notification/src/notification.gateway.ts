...
export class NotificationGateway implements OnGatewayInit {
  ...
  async buscaUsuarios() {
    ...
    try {
      const users = await this.notificationService.obtenerUsuariosDeEventosProximos(currentDate);
      ...
      if (users && users.length > 0) {
        ...
        this.logger.log('No se encontraron usuarios relacionados con eventos próximos.');
      }
    } catch (error) {
